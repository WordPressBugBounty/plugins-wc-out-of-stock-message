...
class Message {
    ...
    public function enqueue_react_for_woocommerce()
	{
		...
		if ( $product && $product instanceof \WC_Product) {		
			...
			if ( is_product() && is_object($product) && method_exists($product, 'is_in_stock')) {			
				$stock_status = $product->get_stock_status();
				$stock_quantity = $product->get_stock_quantity();
				
				if( !$product->is_in_stock() || $stock_quantity < 1 || $stock_status != 'instock' ){
					// Load React and custom JS file
					wp_enqueue_script(
						'wcosm-product',
					...
					$global = get_post_meta($product->get_id(),'_wcosm_use_global_note')?:'';
					if(!$global){
						wp_localize_script( 'wcosm-product', 'wcosm', [
							'data'=> $product->get_id(),
							'nonce' => wp_create_nonce('wcosm'),
							'pid' => $product->get_id(), // Pass the product ID to JavaScript
							'text'=> get_post_meta($product->get_id(),'_out_of_stock_msg')?:'',
						]);
					}
				}

			} else {
				error_log('$product->is_in_stock() not working');
			}
		} 
	}
